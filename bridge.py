...
def scanBlocks(chain):
    ...
    source_contract_address, src_abi = source_contracts["address"], \
                                       source_contracts["abi"]
    destination_contract_address, dst_abi = destination_contracts["address"], \
                                            destination_contracts["abi"]

    source_contract = w3_src.eth.contract(address=source_contract_address,
                                          abi=src_abi)
    destination_contract = w3_dst.eth.contract(
        address=destination_contract_address, abi=dst_abi)
