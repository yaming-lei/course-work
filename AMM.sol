...
contract AMM is AccessControl{
	...
	*/
    ...
    if (sellToken == tokenA) {
        buyAmount = balanceB - (invariant / (balanceA + sellAmountWithFee));
        require(ERC20(tokenA).transferFrom(msg.sender, address(this), sellAmount), 'Transfer of token A failed');
        require(ERC20(tokenB).transfer(msg.sender, buyAmount), 'Transfer of token B failed');
    } else {
        buyAmount = balanceA - (invariant / (balanceB + sellAmountWithFee));
        require(ERC20(tokenB).transferFrom(msg.sender, address(this), sellAmount), 'Transfer of token B failed');
        require(ERC20(tokenA).transfer(msg.sender, buyAmount), 'Transfer of token A failed');
    }

    uint256 invariant1 = ERC20(tokenA).balanceOf(address(this)) * ERC20(tokenB).balanceOf(address(this));
    require(invariant1 >= invariant, 'Bad trade');
...
function provideLiquidity(uint256 amtA, uint256 amtB) public {
    ...
    require(ERC20(tokenA).transferFrom(msg.sender, address(this), amtA), 'Transfer of token A failed');
    require(ERC20(tokenB).transferFrom(msg.sender, address(this), amtB), 'Transfer of token B failed');
...
function withdrawLiquidity(address recipient, uint256 amtA, uint256 amtB) public onlyRole(LP_ROLE) {
    ...
    if (amtA > 0) {
        require(ERC20(tokenA).transfer(recipient, amtA), 'Transfer of token A failed');
    }
    if (amtB > 0) {
        require(ERC20(tokenB).transfer(recipient, amtB), 'Transfer of token B failed');
    }
